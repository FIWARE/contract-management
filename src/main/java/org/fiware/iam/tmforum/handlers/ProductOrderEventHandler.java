...
import org.fiware.iam.tmforum.NotificationListener;
...
public class ProductOrderEventHandler implements EventHandler {
	...
	private static final String UPDATE_EVENT = "ProductOrderUpdateEvent";
	...
	private static final List<String> SUPPORTED_EVENT_TYPES = List.of(CREATE_EVENT, UPDATE_EVENT, DELETE_EVENT, STATE_CHANGE_EVENT);
	...
	public Mono<HttpResponse<?>> handleEvent(String eventType, Map<String, Object> event) {
		...
		return switch (eventType) {
			...
			case UPDATE_EVENT -> handelUpdateEvent(orgId, event);
	...
	private Mono<HttpResponse<?>> handelCreateEvent(String organizationId, Map<String, Object> event) {
		...
		boolean isCompleted = Optional.ofNullable(productOrderCreateEventVO.getEvent())
				.map(ProductOrderCreateEventPayloadVO::getProductOrder)
				.map(ProductOrderVO::getState)
	...
	private Mono<HttpResponse<?>> handelStateChangeEvent(String organizationId, Map<String, Object> event) {
		...
		boolean isCompleted = Stream.ofNullable(productOrderStateChangeEventVO)
				.map(ProductOrderStateChangeEventVO::getEvent)
				.map(ProductOrderStateChangeEventPayloadVO::getProductOrder)
				.map(ProductOrderVO::getState)
				.anyMatch(ProductOrderStateTypeVO.COMPLETED::equals);
